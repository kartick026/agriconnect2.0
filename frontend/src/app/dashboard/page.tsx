                    ...
                    <div className="relative z-10 mt-8 pt-8 border-t border-white/10">
                        <button
                            onClick={() => {
                                const report = `
═══════════════════════════════════════
   AGRICONNECT — FARM STATUS REPORT
   Generated: ${new Date().toLocaleDateString('en-IN', { dateStyle: 'full' })}
═══════════════════════════════════════

MARKET OVERVIEW
───────────────
• Wheat:  ₹2,300/Qtl  (+4.5% from last week)
• Tomato: ₹1,200/Qtl  (-2.1% from last week)

ESTIMATED WALLET PROFIT: ₹45,000+
Based on current selected crops and acreage.
...
RECOMMENDATIONS
───────────────
1. Consider selling Wheat at current high prices.
2. Monitor Tomato prices — downward trend detected.
3. Pest control is scheduled — ensure timely application.

═══════════════════════════════════════
  Report generated by AgriConnect
═══════════════════════════════════════
`.trim();
                                ...
                                a.href = url;
                                a.download = `AgriConnect_Report_${new Date().toISOString().split('T')[0]}.txt`;
                                a.click();
                                URL.revokeObjectURL(url);
                            }}
                            className="w-full flex items-center justify-center px-6 py-4 bg-[#a4e320] hover:bg-[#8ec21b] transition-colors rounded-2xl text-[#004d2b] font-extrabold text-sm uppercase tracking-wider shadow-[0_10px_30px_rgba(164,227,32,0.2)]"
                        >
